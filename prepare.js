...
function getNextEvents(membersData) {
  ...
  if (eventsSource.type !== null) {
    console.log(
      `    · Getting upcoming events json for ${colors.green(
        membersData.name
      )} from ${colors.underline(eventsSource.type)}`
    );
    membersData.nextEvent = memberEvents.getNextEvent(eventsSource);
  } else {
    console.log(
      `    · Skipping upcoming events for ${colors.green(
        membersData.members[group].name
      )}: No source specified`
    );
  }
  // return membersData
...
function getMembersInfo() {
  const source = process.env.MEMBERS_SOURCE_FILE;
...
async function getMembersVideos(membersData) {
  for (let group in membersData.members) {
    ...
    for (let videosSourcesKey in videosSources) {
      ...
      console.log(
        `    · Getting member videos for ${colors.green(
          membersData.members[group].name
        )} from ${colors.underline(videoSource.type)}`
      );
...
console.log(`${colors.inverse('Getting members.json file')}`);
let data = getMembersInfo();

console.log(`${colors.inverse('Validate members.json file')}`);
...
getMembersVideos(data).then(data => {
  saveJsonFile(data);
});
