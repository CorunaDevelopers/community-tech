...
function getNextEvents(data) {
  ...
  eventEmitter.on('getNextFromSourceInit', (source, options) => {
    console.log(
      `    · Getting upcoming events json for ${colors.green(
        options.member.name
      )} from ${colors.underline(source.type)}`
    );
  });
  ...
  eventEmitter.on('getNextFromSourceCompleted', (nextEvents, options) => {
    if (nextEvents.length === 0) {
      ...
      console.log(
        `        ${colors.cyan(`Upcoming event found:`)} ${colors.blue(
          `${colors.bold(`${nextEvent.title}`)} ${eventDate(nextEvent.date)}`
        )}`
      );
    }
    console.log();
...
async function getMembersVideos(membersData) {
  ...
  eventEmitter.on('getVideosFromSourceInit', (source, options) => {
    console.log(
      `    · Getting member videos for ${colors.green(options.member.name)} from ${colors.underline(
        source.type
      )}`
    );
  });
...
console.log(`${colors.inverse('Getting json file')}`);
...
console.log(`${colors.inverse('Validate json file')}`);
